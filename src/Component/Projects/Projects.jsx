					...
					<div className="grid grid-cols-1 gap-x-4 gap-y-8 md:grid-cols-2 lg:grid-cols-3 mx-auto">
						{projects.map((project) => (
							<article
								key={project.id}
								data-aos="fade-up"
								className="hover:bg-orange-50 shadow-lg border border-gray-100 rounded-2xl flex flex-col dark:bg-gray-50"
							>
								<img
								...
								<div className="flex flex-col flex-1 p-6">
									<span className="text-xs tracking-wider uppercase hover:underline dark:text-violet-600 mb-2">
										{project.techStack}
									</span>
									<h3 className="flex-1 py-2 text-lg font-semibold leading-snug">
										{project.name}
									</h3>
									<p className="text-sm text-gray-600 mb-4">{project.brief}</p>
									<button
										onClick={() => setSelectedProject(project)}
										className="self-start px-4 py-2 text-white bg-orange-400 rounded hover:bg-orange-500 focus:outline-none focus:ring-2 focus:ring-orange-400 cursor-pointer"
									>
										View More 
									</button>
								</div>
